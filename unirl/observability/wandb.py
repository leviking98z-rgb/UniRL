"""WandB observer adapter for UniRL training.
...
class UniRLWandBLogger:
    ...
    def log_rollout_step(
        ...
        *,
        ...
        if not self.enabled or not self._initialized:
            return
        # Lazy import keeps the adapter importable without the training stack.
        from unirl.observability.metrics import compute_rollout_sample_metrics
...
def aggregate_stage_results(results: List[Any]) -> Dict[str, float]:
    ...
    if not results:
        return {}
    # Lazy import keeps the adapter importable without pulling in
