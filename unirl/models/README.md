...
same conditions, codecs, geometry, pipeline, and diffusion-kernel behavior.
Together they implement the shared **bundle / pipeline / stage / conditions**
contract so the same model code, on the same weights, serves *both* the rollout
engine (generate) and the train stack (replay).
