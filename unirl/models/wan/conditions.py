"""WANConditions — typed conditions shared by the WAN model family.

Concrete instantiation of the ``DiffusionStage[C]`` type parameter. WAN
2.1 and 2.2 consume a text-conditioning slot with an explicit CFG
negative branch, plus an optional ``image_latent`` slot carrying the
...
unused at the diffusion stage because ``WANTextEmbedStage`` already
...
class WAN21Conditions(Batch):
    """Typed conditions container shared by WAN 2.1 and WAN 2.2."""
...
# Neutral family-level name for new call sites. Keep the historical symbol as
# the class definition so existing recipes, serialized type names, and public
# imports remain unchanged.
WANConditions = WAN21Conditions


__all__ = ["WAN21Conditions", "WANConditions"]
