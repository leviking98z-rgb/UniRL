"""Diffusion model contracts for rollout and post-training.
...
class ReplayResult:
    ...
    ``log_probs`` is always populated; the other fields are stage-specific and
    may be ``None``. Diffusion replay currently populates
    ``prev_sample_means``. The ``logits`` slot is reserved for future AR replay
    unification.
    """

    log_probs: torch.Tensor
    """Aligned with ``segment.sde_logp`` (or its selected slice), shape ``[B, S']``."""

    prev_sample_means: Optional[torch.Tensor] = None
    """Per-step SDE Gaussian means, shape ``[B, S', *latent_shape]``."""

    logits: Optional[torch.Tensor] = None
    """Reserved per-step token logits, shape ``[B, S', V]``."""
...
__all__ = ["DiffusionLatentSpec", "DiffusionStage", "DiffusionStep", "ReplayResult"]
